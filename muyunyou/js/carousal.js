...
*/
(function () {
    var carousal_list = document.getElementById('carousal_list');
    ...
    // 克隆第一张li
    var clone_li = carousal_list.firstElementChild.cloneNode(true);
    // 上树
    carousal_list.appendChild(clone_li);
    ...
    function right_btn_handler() {
        ...
        lock = false;
        // 加上过渡
        carousal_list.style.transition = 'transform .5s ease 0s';
        // idx加1
        idx++;
        // 拉动
        carousal_list.style.transform = 'translateX(' + -16.66 * idx + '%)';
        // 判断是否是最后一张，如果是最后一张，那么就要瞬间移动回来
        if (idx > 4) {
            setTimeout(function () {
                // 去掉过渡
                carousal_list.style.transition = 'none';
                // 删除transform属性
                carousal_list.style.transform = 'none';
                // 全局图片序号变为0
                idx = 0;
            }, 500); 
        }
    ...
    left_btn.onclick = function () {
        ...
        if (idx == 0) {
            ...
            carousal_list.style.transition = 'none';
            // 拉到最后（瞬间移动）
            carousal_list.style.transform = 'translateX(' + -16.66 * 5 + '%)';
            // 改变idx的值
            idx = 4;

            // 小技巧，延时0毫秒非常有用，可以让刚才的瞬间移动发生之后，再把过渡加上
            setTimeout(function () {
                // 加上过渡
                carousal_list.style.transition = 'transform .5s ease 0s';
                // 动画
    ...
    banner.onmouseleave = function () {
        // 设表先关
        clearInterval(timer);
        // 设置定时器，这里不要加var， 加var就变成局部变量了，就clear不掉了
